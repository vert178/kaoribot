...
module.exports = {
	...
	async execute(message, args) {
            ...
            message.channel.send(prompt).then(async prompt1 => {
                ...
                emojiCollector.on('collect', (reaction, user) => {
                    ...
                    messageCollector.on('collect', (input, user) => {
                        promptid = Math.min(data.length + 3, 4);
                        ...
                        prompt1.channel.send(embed(args[0], data)).then(async confirm => {
                            ...
                            emojiCollector.on('collect', (confirmation, user) => {
                                if (emojis.indexOf(confirmation.emoji.name) != 1) {
                                    confirm.delete();
                                    confirm.channel.send(prompttexts[5]);
                                    return;
                                }
                                else {
                                    //Initiates storage
                                    store(args[0], data);
                                    confirm.channel.send(prompttexts[8]);
        ...
        //Storage on file
        var store = async function storeData(cmd, data) {
            var storage = [];
            storage.push(cmd);
            storage.push(false);
            storage.push(message.author.tag);
            var merged = storage.concat(data);
